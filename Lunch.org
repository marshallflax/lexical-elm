...
*** Javascript engines are awfully fast -- especially with js that doesn't do silly things like confuse Ints and Strings
*** DOM interaction is still slow -- the rendering engine keeps being asked to do more and more
*** Elm does double-buffering against a "Virtual DOM", then computes the delta and sends exactly that to the actual DOM
